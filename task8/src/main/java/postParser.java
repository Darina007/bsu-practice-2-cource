...
public interface postParser {
    ...
    default String replaceDateInJson(String json, Date date) {
        String dateStr;
        String reformatDate;
        SimpleDateFormat dateFormat = new SimpleDateFormat(ConstantDictionary.DATE_PATTERN);
        dateStr = String.valueOf(date);
        reformatDate = dateFormat.format(date);
        return json.replaceAll(dateStr, reformatDate);
    }
}
