    ...
    function getPosts(skip, top, filterConfig) {
        ...
        if (filterConfig) {
            ...
            if (filterConfig.validateUntil) {
                workingArray = workingArray.filter(function (value) {
                    return value.validateUntil - filterConfig.validateUntil >= 0;
                })
            }
            if (filterConfig.discount) {
                workingArray = workingArray.filter(function (value) {
                    return value.discount - filterConfig.discount >= 0;
                })
            }
            if (filterConfig.rating) {
                workingArray = workingArray.filter(function (value) {
                    return value.rating - filterConfig.rating >= 0;
                })
            }
        }
